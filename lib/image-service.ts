...
/**
 * Generates multiple image URLs for a component using AI text-to-image, then Unsplash, then Pexels fallback
 */
export async function generateImageUrls(
  ...
  // DISABLED: Text-to-image API generates large base64 data URLs (1-2MB each)
  // which cause "request body too large" errors when sent to LLMs
  // Instead, we use Unsplash/Pexels for the initial generation
  // Users can optionally replace with AI images later via a separate endpoint

  const enableAIImages = false; // Set to true to enable AI image generation
  const textToImageUrl = process.env.TEXT_TO_IMAGE_API_URL;

  if (enableAIImages && textToImageUrl) {
    try {
      const promises = [];
      for (let i = 0; i < count; i++) {
        const prompt = generateContextualImagePrompt(industry, component, i, businessContext);
        const enhancedPrompt = enhancePromptForImageType(prompt, component);

        promises.push(
          fetch(textToImageUrl, {
            method: 'POST',
      ...
      for (const result of responses) {
        if (result.status === 'fulfilled' && result.value.ok) {
          const imageBlob = await result.value.blob();
          ...
          images.push({
            url: dataUrl,
            alt: `AI-generated ${industry} ${component} image`,
            credit: 'AI Generated'
          });
        }
      }

      // If we got enough AI images, return them
      if (images.length >= count) {
        console.log(`Successfully generated ${images.length} AI images for ${industry} ${component}`);
        return images;
      }
    } catch (error) {
      console.warn('Text-to-image API failed, falling back to stock photos:', error);
    }
  }

  // Try Unsplash if we don't have enough images yet
  if (images.length < count && unsplashService.isConfigured() && businessContext) {
    try {
...
/**
 * Enhances prompts with style modifiers based on image type
 */
function enhancePromptForImageType(prompt: string, imageType: string): string {
  let enhancement = '';

  switch (imageType) {
    case 'hero':
      enhancement = ' - high quality, professional photography, bright lighting, modern, clean aesthetic, wide angle, 4k';
      break;
    case 'services':
    case 'service':
      enhancement = ' - service in action, professional setting, high detail, commercial photography, realistic, sharp focus';
      break;
    case 'team':
    case 'about':
      enhancement = ' - professional team photo, business attire, office environment, friendly and approachable, group portrait';
      break;
    case 'facility':
      enhancement = ' - modern facility, clean environment, professional space, architectural photography, interior design';
      break;
    case 'gallery':
      enhancement = ' - showcase image, high quality results, before and after, detailed work, professional photography';
      break;
    default:
      enhancement = ' - professional, high quality, commercial photography, realistic';
  }
...
/**
 * Generates contextual prompts for AI image generation
 */
function generateContextualImagePrompt(
  ...
  const industryKeywords = INDUSTRY_KEYWORDS[industry] || INDUSTRY_KEYWORDS.default;
  const mainKeyword = industryKeywords[0];
  ...
  switch (component) {
    case 'hero':
      basePrompt = `Professional ${mainKeyword} business hero image, modern and welcoming atmosphere, high-quality commercial photography, bright and clean, professional setting`;
      break;
    case 'services':
    case 'service':
      const serviceKeyword = industryKeywords[Math.min(index + 1, industryKeywords.length - 1)] || mainKeyword;
      basePrompt = `${serviceKeyword} service in action, professional workers, modern equipment, detailed and clean, commercial photography, active work scene`;
      break;
    case 'about':
    case 'team':
      basePrompt = `Professional ${mainKeyword} business team, diverse group of professionals, modern office environment, friendly and approachable, business casual attire, team collaboration`;
      break;
    case 'facility':
      basePrompt = `Modern ${mainKeyword} business facility, clean and organized workspace, professional environment, architectural photography, well-lit interior`;
      break;
    case 'gallery':
      basePrompt = `${mainKeyword} work showcase, before and after results, professional quality work, detailed close-up, commercial photography`;
      break;
    default:
      basePrompt = `Professional ${mainKeyword} business image, high quality, modern, commercial photography`;
  }

  if (companyName) {
    basePrompt = `${companyName} - ${basePrompt}`;
  }
