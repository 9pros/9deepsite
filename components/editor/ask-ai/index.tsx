...
export function AskAI({
  ...
  const callAi = async (passedRedesignMarkdown?: string) => {
    ...
    if (isFollowUp && !markdownToUse && !isSameHtml) {
      ...
      if (result?.success) {
        setPrompt("");
        // Keep the same model even for thinker models - thinking will be disabled on backend for follow-ups
      }
    }
  };
          ...
          <div className="absolute top-0 right-0 -translate-y-[calc(100%+8px)] select-none text-xs text-neutral-400 flex items-center justify-center gap-2 bg-neutral-800 border border-neutral-700 rounded-md p-1 pr-2.5">
            ...
            >
              <Checkbox
                ...
                checked={isFollowUp}
                onCheckedChange={(e) => {
                  // Keep the same model even for thinker models - thinking will be disabled on backend
